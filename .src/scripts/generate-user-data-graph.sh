#!/bin/bash
# Generate Mermaid graph markdown for the current Pensieve user data root.
...
END {
    ...
    print "<!-- Generated by: .src/scripts/generate-user-data-graph.sh -->"
    print ""
    print "# Pensieve 用户数据图谱"
    print ""
    print "> Claude Code 环境下自动生成；其他环境编辑 pensieve 后须运行 `bash .src/scripts/maintain-project-skill.sh --event sync` 触发生成。"
    print ""
    print "- 根目录: `" root "`"
