...
> 说明：`kingkongshot-marketplace` 来自本仓库 `.claude-plugin/marketplace.json` 的 `name` 字段。
...
1. 卸载插件：`claude plugin uninstall pensieve@kingkongshot-marketplace --scope user`（或用 `/plugin` UI）
